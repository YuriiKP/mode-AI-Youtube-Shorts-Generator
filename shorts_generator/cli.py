...
def _write_shorts_info(result: Dict, output_dir: str) -> Optional[str]:
    """Write a plain-text sheet with each short's title and description.

    The file is a copy-paste sheet for the publishing fields of the platforms
    (YouTube, TikTok, Instagram, ...): one block per rendered short with its
    title and the short description generated by the LLM. It is written next to
    the rendered clips, so nothing has to be retyped from the console.
    """
    ...
    for video in videos:
        ...
        for short in shorts:
            ...
            if clip:
                lines.append(f"Файл:        {os.path.basename(str(clip))}")
            lines.append("")
        blocks.append("\n".join(lines).rstrip())
