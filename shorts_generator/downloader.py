...
Returns a local mp4 path the rest of the pipeline can read directly off disk.
"""
...
def _resolve_local_path(source: str) -> Optional[str]:
    ...
    if (
        any(sep in source for sep in (os.sep, "/"))
...
def download_youtube(
    ...
    if local_path:
        print(f"[download] using local file: {local_path}", flush=True)
        return local_path
    ...
    if video_id:
        ...
        if cached:
            print(f"[download] reusing cached download: {cached}", flush=True)
            return cached
