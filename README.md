...
- **Цветокоррекция и эффекты**. Накладываются на само видео *до* субтитров и
  баннера, поэтому текст остаётся чётким. Настройки: `SATURATION` (множитель
  насыщенности: `1.0` — как в исходнике, `0` — ч/б, значения выше `1` делают цвет
  ярче; по умолчанию `1.0`), `SHARPNESS` (сила нерезкого маскирования: `0`
  отключает, `1.0` — мягко, `2.0` — сильно; по умолчанию `0`) и
  `CHROMATIC_ABERRATION` (смещение красного и синего каналов в пикселях к углам
  кадра — цветная «каёмка» по краям; `0` отключает). Все три параметра по
  умолчанию нейтральны и не меняют картинку.
