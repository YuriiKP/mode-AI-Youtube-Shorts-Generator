...
description: Generate viral 9:16 YouTube Shorts (or TikTok/Reels clips) from a long-form YouTube URL or local video. Triggers on requests like "make shorts from this video", "extract viral clips from this YouTube link", "auto-clip this podcast", "find the best moments and crop vertical". Pipeline downloads the source, transcribes locally with faster-whisper, ranks highlights through a virality framework (hook / emotional peak / opinion bomb / revelation / conflict / quotable / story peak / practical value), dedupes overlapping candidates, and vertically auto-crops the top N as mp4s.
---
...
- Python 3.10+
- `ffmpeg` on PATH
- An LLM API key — set `OPENAI_API_KEY`, `DEEPSEEK_API_KEY`, or `GEMINI_API_KEY` in `.env` (select the provider with `LLM_PROVIDER`). Only the highlight-ranking step is remote. If missing, stop and ask the user for it; do not invent one.
...
1. **Download** (`downloader.py`) — pull the source video with `yt-dlp` at the requested resolution (`360`/`480`/`720`/`1080`, default `720`); a local path or `file://` URL is used directly.
2. **Transcribe** (`transcriber.py`) — `faster-whisper` runs locally (CPU or CUDA) and returns timestamped segments.
...
8. **Cut + vertical crop** (`clipper.py`) — cut each highlight with `ffmpeg` and reframe to `aspect_ratio` using OpenCV face tracking (fall back to a static centre crop with `--no-face-tracking`).
...
- `shorts_generator/config.py` (or env vars)
  - `LLM_PROVIDER` — `openai` (default), `deepseek`, or `gemini`
  - `WHISPER_MODEL` — `base` (default)
  - `WHISPER_DEVICE` — `auto` (default), `cpu`, or `cuda`

## Whisper transcription

Audio is transcribed locally by `faster-whisper`. Set the model with `WHISPER_MODEL` and the device with `WHISPER_DEVICE` (`auto` picks CUDA when available). The CLI passes `--language` straight through; leave it empty for auto-detection, or pass an ISO-639-1 code (e.g. `en`) to lock it.
...
- **LLM key missing or rejected** — surface the exact error; never fabricate a key.
- **`ffmpeg`/`yt-dlp`/`faster-whisper` missing** — surface the install hint the tool prints; don't attempt the step without them.
...
If a rendered clip file is missing, re-run just the crop stage for that highlight rather than re-running the whole pipeline.
